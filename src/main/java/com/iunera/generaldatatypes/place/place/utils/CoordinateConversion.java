...
public class CoordinateConversion {
  ...
  private class LatLon2UTM {
    ...
    protected void setVariables(double latitude, double longitude) {
      ...
      this.rho =
          this.equatorialRadius
              * (1 - this.e * this.e)
              / POW(1 - POW(this.e * SIN(latitude), 2), 3 / 2.0);
      ...
      this.S =
          this.A0 * latitude
              - this.B0 * SIN(2 * latitude)
              + this.C0 * SIN(4 * latitude)
              - this.D0 * SIN(6 * latitude)
              + this.E0 * SIN(8 * latitude);

      this.K1 = this.S * this.k0;
      this.K2 =
          this.nu * SIN(latitude) * COS(latitude) * POW(this.sin1, 2) * this.k0 * (100000000) / 2;
      this.K3 =
          ((POW(this.sin1, 4) * this.nu * SIN(latitude) * Math.pow(COS(latitude), 3)) / 24)
              * (5
                  - POW(TAN(latitude), 2)
                  + 9 * this.e1sq * POW(COS(latitude), 2)
                  + 4 * POW(this.e1sq, 2) * POW(COS(latitude), 4))
              * this.k0
              * (10000000000000000L);
      ...
      this.K5 =
          POW(this.sin1 * COS(latitude), 3)
              * (this.nu / 6)
              * (1 - POW(TAN(latitude), 2) + this.e1sq * POW(COS(latitude), 2))
              * this.k0
              * 1000000000000L;

      this.A6 =
          (POW(this.p * this.sin1, 6) * this.nu * SIN(latitude) * POW(COS(latitude), 5) / 720)
              * (61
                  - 58 * POW(TAN(latitude), 2)
                  + POW(TAN(latitude), 4)
                  + 270 * this.e1sq * POW(COS(latitude), 2)
                  - 330 * this.e1sq * POW(SIN(latitude), 2))
              * this.k0
              * (1E+24);
    }
    ...
    protected double getNorthing(double latitude) {
      double northing = this.K1 + this.K2 * this.p * this.p + this.K3 * POW(this.p, 4);
      if (latitude < 0.0) {
    ...
    // Mean radius
    double rm = POW(this.equatorialRadius * this.polarRadius, 1 / 2.0);

    // scale factor
    double k0 = 0.9996;

    // eccentricity
    double e = Math.sqrt(1 - POW(this.polarRadius / this.equatorialRadius, 2));

    double e1sq = this.e * this.e / (1 - this.e * this.e);

    double n =
        (this.equatorialRadius - this.polarRadius) / (this.equatorialRadius + this.polarRadius);
  ...
  private class UTM2LatLon {
    ...
    protected void setVariables() {
          ...
          this.arc
              / (this.a
                  * (1
                      - POW(this.e, 2) / 4.0
                      - 3 * POW(this.e, 4) / 64.0
                      - 5 * POW(this.e, 6) / 256.0));

      this.ei =
          (1 - POW((1 - this.e * this.e), (1 / 2.0))) / (1 + POW((1 - this.e * this.e), (1 / 2.0)));
      ...
      this.phi1 =
          this.mu
              + this.ca * SIN(2 * this.mu)
              + this.cb * SIN(4 * this.mu)
              + this.cc * SIN(6 * this.mu)
              + this.cd * SIN(8 * this.mu);

      this.n0 = this.a / POW((1 - POW((this.e * SIN(this.phi1)), 2)), (1 / 2.0));

      this.r0 =
          this.a * (1 - this.e * this.e) / POW((1 - POW((this.e * SIN(this.phi1)), 2)), (3 / 2.0));
      ...
      this.fact3 =
          (5 + 3 * this.t0 + 10 * this.Q0 - 4 * this.Q0 * this.Q0 - 9 * this.e1sq)
              * POW(this.dd0, 4)
              / 24;
  ...
  private class Digraphs {
    ...
    private String[] digraph2Array = {
      "V", "A", "B", "C", "D", "E", "F", "G", "H", "J", "K", "L", "M", "N", "P", "Q", "R", "S", "T",
      "U", "V"
    };
    ...
    public int getDigraph1Index(String letter) {
      for (int i = 0; i < this.digraph1Array.length; i++) {
        if (this.digraph1Array[i].equals(letter)) return i + 1;
      }
    ...
    public int getDigraph2Index(String letter) {
      for (int i = 0; i < this.digraph2Array.length; i++) {
        if (this.digraph2Array[i].equals(letter)) return i;
      }
  ...
  private class LatZones {
    private char[] letters = {
      'A', 'C', 'D', 'E', 'F', 'G', 'H', 'J', 'K', 'L', 'M', 'N', 'P', 'Q', 'R', 'S', 'T', 'U', 'V',
      'W', 'X', 'Z'
    };
    ...
    public int getLatZoneDegree(String letter) {
      ...
      for (int i = 0; i < this.arrayLength; i++) {
        if (this.letters[i] == ltr) return this.degrees[i];
      }
      return -100;
    }
